<template>
    <div class="modal-container add-coupon-modal" v-if="modalStatus">
        <el-dialog title="新建优惠卷" :visible.sync="modalStatus" @close='handleCloseModal' >
...
<script>
    import { requestAddCoupon } from '@/services/service';
    ...
    export default {
        ...
        computed: {
            modalStatus: {
                get(){
                    return (this.$store.state.addCouponModalStatus || false)
                },
                set(){
        ...
        watch: {
            modalStatus(oldVal,newVal){
                this.modalStatus = this.$store.state.addCouponModalStatus
            },
            timePicker(){
        ...
        methods: {
            handleConfirm(){
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        let params = Object.assign({},this.form,typeConifg,{
                            shopId: this.$route.params.id,
                            remainCount: this.form.allCount
                        });
                        requestAddCoupon(params).then(function(res){
            ...
            handleCloseModal(){
                ...
                this.timePicker = '';
                this.$store.dispatch('closeAddCouponModal')
            },
            handleChange(val){
