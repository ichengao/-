<template>
    <div class="modal-container edit-store-control-modal" v-if="modalStatus">
...
<script>
    import { requestUpdatewarehouse } from '@/services/service';
    ...
    export default {
        ...
        computed: {
            modalStatus: {
                get(){
                    return (this.$store.state.editStoreControlModalStatus.status || false)
                },
                set(){
        ...
        watch: {
            modalStatus(oldVal,newVal){
                this.modalStatus = this.$store.state.editStoreControlModalStatus.status;
                if(this.modalStatus){
                    let data = this.$store.state.editStoreControlModalStatus.data;
                    this.form = {
                        name: data.name,
                        admin: data.admin,
                        tel: data.tel,
        ...
        methods: {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        let params = {
                            ...
                            warehouseId: this.$store.state.editStoreControlModalStatus.data.warehouseId
                        };
                        params = Object.assign({},this.form,params);
                        requestUpdatewarehouse(params).then(function(res){
            ...
            handleCloseModal(){
                ...
                this.$store.dispatch('closeEditStoreControlModal')
            }
        }
    }
</script>
