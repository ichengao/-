<template>
    <div class="modal-container change-pwd-modal" v-if="modalStatus">
        <el-dialog title="新增商品分类" :visible.sync="modalStatus" @close='handleCloseModal' >
...
<script>
    ...
    export default {
        ...
        computed: {
            modalStatus: {
                get(){
                    return (this.$store.state.createPruductCategoryStatus || false)
                },
                set(){
        ...
        watch: {
            modalStatus(oldVal,newVal){
                this.modalStatus = this.$store.state.createPruductCategoryStatus
            }
        },
        methods: {
            handleConfirm(){
            ...
            handleCloseModal(){
                ...
                this.$store.dispatch('closePruductCategoryModal')
            }
        }
    }
</script>
