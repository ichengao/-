<template>
    <div class="modal-container create-supplier-vendor-modal" v-if="modalStatus">
        <el-dialog title="新增供应商" :visible.sync="modalStatus" @close='handleCloseModal' >
...
<script>
    import { requestAddsupplier } from '@/services/service';
    ...
    export default {
        ...
        computed: {
            modalStatus: {
                get(){
                    return (this.$store.state.createSupplierVendorModalStatus|| false)
                },
                set(){
        ...
        watch: {
            modalStatus(oldVal,newVal){
                this.modalStatus = this.$store.state.createSupplierVendorModalStatus
            }
        },
        methods: {
            handleConfirm(){
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        ...
                        requestAddsupplier(params).then(function(res){
            ...
            handleCloseModal(){
                ...
                this.$store.dispatch('closeCreateSupplierVendorModal')
            }
        }
    }
</script>
