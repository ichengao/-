<template>
    <div class="modal-container change-pwd-modal" v-if="modalStatus">
        <el-dialog title="修改密码" :visible.sync="modalStatus" @close='handleCloseModal' >
...
export default {
    ...
    computed: {
        modalStatus: {
            get(){
                return (this.$store.state.changePwdModalStatus1 || false)
            },
            set(){
    ...
    watch: {
        modalStatus(oldVal,newVal){
            this.modalStatus = this.$store.state.changePwdModalStatus1
        }
    },
    methods: {
        handleConfirm(){
        ...
        handleCloseModal(){
            ...
            this.$store.dispatch('closeChangePwdModal')
        }
    }
}
</script>
